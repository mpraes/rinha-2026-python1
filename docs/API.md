...
**Critical:** HTTP errors are weighted 5x worse than false positives in the scoring system. Your API must avoid returning `4xx` or `5xx` errors on the fraud detection path.

When in doubt, return a safe response with `HTTP 200`:
...
This applies to:
- Parse failures (malformed JSON, missing fields)
- Wrong HTTP methods
- Unknown routes
- Internal errors
